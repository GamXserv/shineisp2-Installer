...
use Zend\Db\Adapter\Adapter, Zend\Db\TableGateway\TableGateway, Zend\Db\RowGateway\RowGateway, Zend\Db\TableGateway\Feature, Product\Model\ValuesCache;
...
class Eav {
	...
	public function __construct(TableGateway $table) {
		...
		$this->_attributesTable = new TableGateway ( $this->_attributesTableName, $table->getAdapter (), new Feature\RowGatewayFeature ( 'id' ) );
	}
	...
	public function getTypeTable($attribute) {
		...
		if (! isset ( $this->_eavTables [$type] )) {
			$eavTable = new TableGateway ( $this->getTypeTableName ( $type ), $this->_entitiesTable->getAdapter (), new Feature\RowGatewayFeature ( 'id' ) );
	...
	public function getAttribute($id) {
		if (isset ( $this->_attributes [$id] )) {
			...
			$where = array (
					$this->_attributesTableFieldId => $id 
			);
			$attribute = $this->_attributesTable->select ( $where )->current ();
		} else {
			$where = array (
					$this->_attributesTableFieldName => $id 
			);
			$attribute = $this->_attributesTable->select ( $where )->current ();
		}
	 ...
	 *        	entity object
	 * @param RowGateway $attribute
	 *        	attribute object
	 * @param ValuesCache $valuesCache
	 *        	try to use values cache
	 * @param boolean $loadIfNotInCache
	 *        	do query if value is not in cache
	 * @return mixed
	 */
